...
def trainmodel(x_train_1docs, y_train_1docs):
    ...
    for i,item in enumerate(x_train_1docs):
        type = y_train_1docs[i].type
        for word in item:
            categoryDocNum[type] += item[word]
            totalDocNum += item[word]
    ...
    for i,map in enumerate(x_train_1docs):
        ...
        for item1 in map:
            if type in categoryFeatureP:
                if item1 in categoryFeatureP[type]:
                    categoryFeatureP[type][item1] += map[item1]
                elif item1 not in categoryFeatureP[type]:
    ...
    for key, value in categoryDocNum.items():
        defaultP[key] = 0.1 / (value+0.1*len(wordHash))
    ...
    for type1, value1 in categoryFeatureP.items():
        for word in wordHash:
            ...
            if word in value1:
                probability = (value1[word]+0.1)/(typeCount+0.1*len(wordHash))
